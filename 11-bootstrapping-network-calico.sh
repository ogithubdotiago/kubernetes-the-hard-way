...
printc "\n# Criando IP pool\n"
cat > $PATH_CONFIG/ippool.yaml <<EOF
...
cat > $PATH_CONFIG/clusterrole-calico-cni.yaml <<EOF
kind: ClusterRole
...
cat > $PATH_CONFIG/10-calico.conflist <<EOF
{
...
cat > $PATH_CONFIG/clusterrole-calico-typha.yaml <<EOF
kind: ClusterRole
apiVersion: rbac.authorization.k8s.io/v1
metadata:
  name: calico-typha
...
cat > $PATH_CONFIG/deployment-calico-typha.yaml <<EOF
apiVersion: apps/v1
kind: Deployment
metadata:
  name: calico-typha
...
cat > $PATH_CONFIG/service-calico-typha.yaml <<EOF
apiVersion: v1
kind: Service
metadata:
  name: calico-typha
...
cat > $PATH_CONFIG/clusterrole-calico-node.yaml <<EOF
kind: ClusterRole
apiVersion: rbac.authorization.k8s.io/v1
metadata:
  name: calico-node
...
cat > $PATH_CONFIG/daemonset-calico-node.yaml <<EOF
kind: DaemonSet
apiVersion: apps/v1
metadata:
  name: calico-node
...
cat > $PATH_CONFIG/peer-to-rrs.yaml <<EOF
kind: BGPPeer
apiVersion: projectcalico.org/v3
metadata:
  name: peer-to-rrs
...
cat > $PATH_CONFIG/rrs-to-rrs.yaml <<EOF
kind: BGPPeer
apiVersion: projectcalico.org/v3
metadata:
  name: rrs-to-rrs
...
cat > $PATH_CONFIG/bgpconfiguration-default.yaml <<EOF
 apiVersion: projectcalico.org/v3
